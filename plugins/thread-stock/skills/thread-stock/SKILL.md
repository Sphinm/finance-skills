...
| **"帮我选票""有什么可以买的""明天买什么"** | **运行 `scripts/weekly_scan.py` → 加载 `references/screening-workflow.md`** |
| **"周度扫描""帮我做个检查""持仓怎么样"** | **运行 `scripts/weekly_scan.py --holdings XXX` → 加载 `references/weekly-checklist.md`** |
| **"板块资金""哪个板块在流入"** | **运行 `scripts/weekly_scan.py --sector-only`** |
| **"短线""做T""快进快出""有什么异动"** | **运行脚本 → 重点看【短线候选】+ 加载 `references/strategy-by-horizon.md` 短线部分** |
| **"中长线""长期持有""定投""底部布局"** | **运行脚本 → 重点看【长线候选】+ 加载 `references/strategy-by-horizon.md` 长线部分** |
| **"仓位怎么分配""长短线比例"** | **加载 `references/strategy-by-horizon.md` 仓位配置原则** |

## Scripts

### 选股扫描 (`scripts/weekly_scan.py`)

```bash
# 完整扫描（板块资金 + 全池个股，输出短/中/长线三层候选）
...
**选股时必须先运行脚本**，不要凭印象手动挑票。脚本输出三层候选（短线/中线/长线），策略规则详见 `references/strategy-by-horizon.md`。

### 数据采集 (`scripts/fund_flow_collector.py`)
...
**运行时机**: 用户每次让 AI 选票/看盘时，先跑 collector 再跑 scan。不需要 cron——每次聊天时顺手采集。

**漏了几天没开电脑？** 个股行情可以自动回填（yfinance 支持历史），板块资金流无法补（接口只有实时快照）。脚本每次运行会自动检测并回填最近 30 天缺失的个股数据。

数据库位置: `data/fund_flow.db`，`weekly_scan.py` 会自动读取历史数据做趋势对比。
...
6. **脚本优先**：选股时先跑 `weekly_scan.py`，禁止凭印象手动列候选。
